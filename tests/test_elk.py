...
def test_unsupported_platform():
    manager = ELKManager()
    with mock.patch("platform.system", return_value="SolarOS"), \
...
def test_process_management():
    ...
    mock_process.stdout.readline.return_value = "--- ELK layouter started ---\n"
...
def test_failed_process_start():
    ...
    with mock.patch("subprocess.Popen", return_value=mock_process), \
         pytest.raises(RuntimeError, match="Failed to start elk.js helper process"):
        manager._spawn_process_deno()
